  ...
  _.forEach(
    ...
    (connection, name) => {
      ...
      try {
        ...
        siapi.log.error('The client `' + connection.settings.client + '` is not installed.');
        siapi.log.error(
      ...
      const options = _.defaultsDeep(
        ...
        siapi.config.hook.settings.knex,
        defaultConfig
      );

      // Resolve path to the directory containing the database file.
      const fileDirectory = options.connection.filename
        ? path.dirname(path.resolve(siapi.config.appPath, options.connection.filename))
        : '';
      ...
      // please drop us an email at support@siapi.io-- it would avoid the Siapi
      // applications to have `knex` as a dependency.
      try {
        // Try to require from local dependency.
        const connection = require('knex')(options);
        _.set(siapi, `connections.${name}`, connection);
      } catch (err) {
        siapi.log.error('Impossible to use the `' + name + '` connection...');
        siapi.log.warn(

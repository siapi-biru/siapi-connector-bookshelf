...
const populateComponent = (key, attr, options = {}) => {
  ...
  if (attr.type === 'dynamiczone') {
    ...
    return componentKeys.reduce((acc, key) => {
      const component = siapi.components[key];
  ...
  const component = siapi.components[attr.component];
  ...
  const finalObj = Object.keys(obj).reduce((acc, key) => {
    ...
    for (let part of parts) {
      ...
      if (isComponent(tmpModel, part)) {
        ...
        tmpModel = siapi.components[attr.component];
      ...
      tmpModel = siapi.db.getModelByAssoc(assoc);
...
const formatPolymorphicPopulate = ({ assoc }, options = {}) => {
  ...
  const model = siapi.db.getModelByAssoc(assoc);

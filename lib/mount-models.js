...
const { models: utilsModels, contentTypes: contentTypesUtils } = require('siapi-utils');
...
module.exports = async ({ models, target }, ctx, { selfFinalize = false } = {}) => {
  ...
  const updateModel = async model => {
    ...
    if (!definition.uid.startsWith('siapi::') && definition.modelType !== 'component') {
    ...
    Object.keys(definition.attributes).forEach(name => {
      ...
      if (globalName !== '*') {
        globalId = siapi.db.getModel(globalName.toLowerCase(), details.plugin).globalId;
      }

      switch (verbose) {
        case 'hasOne': {
          const target = details.plugin
            ? siapi.plugins[details.plugin].models[details.model]
            : siapi.models[details.model];
        ...
        case 'belongsToMany': {
          const targetModel = siapi.db.getModel(details.collection, details.plugin);
        ...
        case 'morphMany': {
          const collection = details.plugin
            ? siapi.plugins[details.plugin].models[details.collection]
            : siapi.models[details.collection];
    ...
    try {
          ...
          switch (type) {
            ...
            case 'dynamiczone': {
              attrs[key] = relations[key].toJSON().map(el => {
                const componentKey = Object.keys(siapi.components).find(
                  key => siapi.components[key].collectionName === el.component_type
                );

                return {
                  __component: siapi.components[componentKey].uid,
                  ...el.component,
                };
              });
          ...
          if (relation) {
            ...
            switch (association.nature) {
              ...
              case 'oneMorphToOne': {
                ...
                const contentType = siapi.db.getModelByCollectionName(
      ...
      // Expose ORM functions through the `siapi.models[xxx]`
      // or `siapi.plugins[xxx].models[yyy]` object.
      ...
      return async () => {
        try {
          ...
          if (['ER_TOO_LONG_IDENT'].includes(err.code)) {
            siapi.stopWithError(
              err,
              `A table name is too long. If it is the name of a join table automatically generated by Siapi, you can customise it by adding \`collectionName: "customName"\` in the corresponding model's attribute.`
            );
          }
      ...
      if (err instanceof TypeError || err instanceof ReferenceError) {
        siapi.stopWithError(err, `Impossible to register the '${model}' model.`);
      }

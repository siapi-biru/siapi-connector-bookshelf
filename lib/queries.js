...
const { convertRestQueryParams, buildQuery, escapeQuery } = require('siapi-utils');
...
module.exports = function createQueryBuilder({ model, siapi }) {
  /* Utils */
  // association key
  ...
  const wrapTransaction = (fn, { transacting } = {}) => {
    const db = siapi.connections[model.connection];
    ...
    for (let key of componentKeys) {
      ...
      switch (type) {
        case 'component': {
          ...
          const componentModel = siapi.components[component];
  ...
  async function deleteDynamicZoneOldComponents(entry, values, { key, joinModel, transacting }) {
    ...
    const allIds = await joinModel
      ...
      .map(el => {
        const componentKey = Object.keys(siapi.components).find(
          key => siapi.components[key].collectionName === el.get('component_type')
        );

        return {
          id: el.get('component_id').toString(),
          component: siapi.components[componentKey],
        };
      });
    ...
    if (idsToDelete.length > 0) {
      ...
      for (const idToDelete of idsToDelete) {
        ...
        const model = siapi.query(component.uid);
      ...
      switch (type) {
        ...
        case 'dynamiczone': {
          ...
          for (const compo of components) {
            const { uid, collectionName } = siapi.components[compo];
            const model = siapi.query(uid);
  ...
  async function fetchRelationCounters(attribute, entitiesIds = []) {
    ...
    const knex = siapi.connections[model.connection];
